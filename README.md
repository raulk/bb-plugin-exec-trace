...
- `server.ts` — the backend: a todo store in `bb.storage.kv`, RPC methods
  for the page, a `bb exec-trace` CLI command, a setting, and a realtime signal
...
- `skills/example-todos/SKILL.md` — a skill that tells agents how to keep the list
  with `bb exec-trace`. BB imports it into agent threads automatically.
...
`bb exec-trace add "Ship it"` in a terminal. The page updates at once.
...
```
bb plugin config exec-trace
bb plugin config exec-trace set showDone false
bb plugin reload exec-trace
```
